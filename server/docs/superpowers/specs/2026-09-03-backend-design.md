...
- **메일** (`mail/`): `Mailer.send(MailMessage) → provider_message_id`. `smtp`는 aiosmtplib(STARTTLS/SSL 환경 변수), `mock`은 로그 출력 + 가짜 ID. 헤더는 명세 §8.1 그대로(`From: "Fairway ({email})" <MAIL_FROM>`, `Sender`·`Reply-To`=가입 이메일). 본문 말미 고정 문구 추가. 첨부 합계 25MB 초과면 영상 제외 + 안내 한 줄.
...
DATABASE_URL=postgresql+asyncpg://fairway:fairway@localhost:5432/fairway
...
MAIL_FROM=no-reply@fairway.click
FFPROBE_BIN=ffprobe
```
