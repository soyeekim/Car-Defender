...
| `chat` | 한 턴에 질문 하나. 사실 수집 → 더 물을 것이 없으면 "추가 정황 있나요?" → "예상 과실비율을 판정해 드릴까요?" 제안 → 사용자가 요청하면 유사 심의사례 검색 후 `next_action: verdict`(재판정이면 `rejudge`), 문서 요청은 `create_report`/`create_rebuttal`. 대화 중에는 영상을 다시 부르지 않는다(사용자가 명시 요청할 때만). |
