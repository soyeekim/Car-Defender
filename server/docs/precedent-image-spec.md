...
인정기준 도표(심의번호가 `차1-1` `보1` `회전-4` 같은 것)일 때는 앞부분이 `사고 유형` `차량 역할` `사고 내용` `적용 변형`(변형이 둘 이상일 때) 또는 `기본 과실비율` `수정요소` 로 바뀌고, 첫 문단은 `과실비율 인정기준 도표 차1-1`(회전교차로 표는 `회전교차로 비정형기준 도표 회전-4`) 처럼 제목이다. 그 도표로 판정을 계산했으면 `내 사건과 다른 점` 뒤에 `계산 과정` 소제목과 계산식 문단(`기본 (나) A 30 : B 70 → B 중대한 과실 +20 → A 10 : B 90 → 나(B) 90 : 상대(A) 10`)이 붙는다. `기본 과실비율` 문단은 `A:B = 40:60` 형식이다.
...
const CASE_LABELS = ['사고 유형', '사고 내용', '쟁점', '과실비율', '심의 이유', '적용 수정요소', '참고 인정기준', '기본 과실비율', '수정요소', '차량 역할', '적용 변형', '계산 과정'];
