...
직전 턴에 Agent가 사용자에게 한 질문(있는 경우, [field] 질문 형식):
{{pending_questions}}
...
- 사고 날짜(accident_datetime.date)는 사용자가 말한 경우에만 추출한다. "어제", "그제", "3일 전" 같은 상대 표현은
  위 오늘 날짜를 기준으로 YYYY-MM-DD 로 바꾼다. 사용자가 날짜를 말하지 않았으면 날짜를 추출하지 않는다 — 지어내지 않는다.
...
- 하나의 메시지에 여러 질문의 답이 있으면 각각 별도 항목으로 추출한다.
...
ego_vehicle.signal
ego_vehicle.turn_signal (true | false)
ego_vehicle.lane_change (true | false)
ego_vehicle.braking (true | false)
ego_vehicle.entered_first (true | false)
...
other_vehicle.turn_signal (true | false)
other_vehicle.lane_change (true | false)
other_vehicle.braking (true | false)
other_vehicle.entered_first (true | false)
collision.type
...
출력 JSON:
{
  "new_facts": [
    ...
    },
    {
      "field": null,
      "value": "false",
      "fact": "상대 차량이 차로를 변경한 지점은 실선 구간이 아니었다는 사용자 진술",
      "confidence": 0.8,
      "verification": "not_visible_in_video",
      "answers_field": "review.solid_line_lane_change"
    }
  ],
  "conflicts": [
    {
