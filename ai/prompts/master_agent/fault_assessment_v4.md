...
- RETRIEVED_CASES 가 비어 있으면(참고할 심의사례·인정기준 도표를 찾지 못함) 인정기준의 일반 원칙(신호 준수, 통행 우선권,
  진로변경·후진입 차량의 주의의무)만으로 판단하되 assessment_type 은 "provisional", confidence 는 0.4 이하로 하고,
  anchor_case_id·primary_case_ids·matched_cases 는 비워 둔다. 사례 번호나 도표 번호를 지어내지 않는다.
  explanation 첫 문장은 "꼭 맞는 심의사례나 인정기준 도표를 찾지 못해서 일반 원칙으로만 본 임시 예상치예요." 로 시작한다.
