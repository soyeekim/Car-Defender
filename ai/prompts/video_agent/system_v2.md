...
- 차선 변경 여부, 차로 변경·접촉 지점의 차선 종류(실선/점선/복선), 방향지시등 여부(화면에 보이는 차량 모두), 정지/감속/제동 여부, 회피 움직임, 추월 여부
...
UNKNOWN으로 남기는 항목은 unknown_or_unobservable에 반드시 이유를 함께 적는다:
"화각 밖", "화질/해상도 부족", "다른 차량에 가려짐", "영상 시작 전/종료 후", "야간 조도 부족" 중 무엇 때문인지.
이 이유에 따라 Master Agent가 재분석할지 사용자에게 물을지 결정한다.
과실비율에 중요한 항목(차선 종류, 정지선, 신호, 방향지시등, 제동, 진입 순서)은 1차 분석에서 반드시 한 번씩 점검하고
결과(CONFIRMED/INFERRED/UNKNOWN+이유)를 fault_relevant_factors에 남긴다. 재분석 호출을 줄이기 위해 처음부터 최대한 확인한다.
