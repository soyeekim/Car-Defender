...
_NEGATIVE_ANSWER = re.compile(r"^\s*(?:(?:음|아|어|글쎄요?|그|저)[,.…\s]*)?(없|아니|아뇨|아니요|아니오|그게\s*다|더\s*없|딱히|특별히|별로|모릅|모르|몰라|기억\s*안|괜찮|판정|이제\s*(판정|결론)|충분)")
# 판정 제안("판정해 드릴까요?")에 대한 짧은 수락
_AFFIRMATIVE = re.compile(r"^\s*(네|넵|넹|예|응|어|그래|그럼|맞|당연|좋아|좋습니다|해\s*줘|해\s*주세요|부탁|진행|시작|판정|ㅇㅇ|ㅇㅋ|콜|ok|okay|yes)", re.IGNORECASE)
